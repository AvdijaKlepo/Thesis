...
These experiments reuse the existing Docker fixtures, proxy configurations, scenario runner, analyzer, and dashboard. No application code or new backend image is needed. Each study answers a different load-balancing question; failure recovery remains a separate availability study.

## Choose a study

Every manifest compares `round_robin`, `weighted_round_robin`, `least_connections`, and `least_response_time` across `thread_pool` and `async`, with three repetitions by default.
...
The six new manifests contain nine scenarios and 216 matrix cells in total. Including failure recovery gives 240. Run a smoke subset first; these defaults are starting protocols, not a claim that three repetitions or ten seconds of arrivals are sufficient for every thesis conclusion.
...
Do not manually launch the proxy or fixtures for these automated experiments. The runner owns their startup and teardown. Keep proxy/control/admin ports 7879/7878/7880 and the selected profile's fixture ports free (5101-5103, 5201-5203, 5301-5303, or 5401-5403). If you already run that profile manually, stop its specific containers yourself or make an isolated copy with different ports in both the Compose file and server/collection configuration.

Each scenario uses a dedicated Compose project named `webserver-benchmark-{scenario}`. Setup builds and recreates only its three selected fixtures, waits for them, then starts a fresh proxy. Teardown removes that scenario's Compose project, without targeting your normal `webserver-fixtures` project. Project names isolate lifecycle, **not published ports**: run these experiments sequentially. An existing listener on a required port is an error, not a request to stop another application.
...
To run the entire new suite sequentially in PowerShell:

```powershell
$studies = @(
    'equal-capacity', 'heterogeneous-capacity', 'variable-latency',
    'saturation-sweep', 'burst-load', 'retry-behavior'
)
