...
impl StaticFileHandler {
    ...
    pub fn serve(&self, request_path: &str) -> io::Result<Response> {
        ...
        match fs::read(&file_path) {
            Ok(contents) => {
                let content_type = content_type(file_path.to_string_lossy().as_ref());

                Ok(Response::ok(content_type, contents))
            }

            Err(error) if error.kind() == io::ErrorKind::NotFound => self.not_found(),
            Err(error) => Err(error),
        }
    }
    ...
    fn resolve(&self, request_path: &str) -> Option<PathBuf> {
        let path = match request_path {
            ...
            path if path.starts_with('/') => path.trim_start_matches('/'),
...
impl Response {
    ...
    pub fn to_http(&self) -> Vec<u8> {
        ...
        let mut response = Vec::with_capacity(headers.len() + self.body.len());
